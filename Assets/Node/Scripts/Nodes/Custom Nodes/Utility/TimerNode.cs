...
public class TimerNode : BaseInputNode {
    ...
    public override void DrawWindow() {
        ...
        if(!enableWait) { status = "Seconds to disable: " + ( disabledSeconds - statusTimer ); }
    ...
    public override void Tick(float deltaTime) {
        if(enableWait) {
            if(statusTimer < enabledSeconds) { statusTimer += deltaTime; } 
            else {
                statusTimer = 0;
                enableWait = false;
                currentResult = true;
            ...
            if(statusTimer < disabledSeconds) { statusTimer += deltaTime; } 
            else {
                statusTimer = 0;
                enableWait = true;
                currentResult = false;
