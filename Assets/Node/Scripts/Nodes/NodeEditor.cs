...
public class NodeEditor : EditorWindow
{
    ...
    private Vector2 scrollStartMousePos, mouseDiff;
    ...
    float panY, totalPanY;
    float panX, totalPanX;
    ...
    void OnGUI() {
        ...
        EditorGUILayout.Space();
        EditorGUILayout.Space();
        _debug = EditorGUILayout.ToggleLeft("Debug", _debug, GUILayout.MaxWidth(55));
        ...
        if (scrollWindow) {
            ...
            mouseDiff.Normalize();
            if (mouseDiff.x != 0) panX += mouseDiff.x;
            if (mouseDiff.y != 0) panY += mouseDiff.y;
            //Debug.Log(panX + " " + panY);
            Debug.Log(mouseDiff);
        } else if (mouseDiff == Vector2.zero) {
        ...
        if ((Event.current.rawType == EventType.MouseUp) && (GUIUtility.hotControl != _mainwindowID)) { GUIUtility.hotControl = 0; }
        ...
        if (_debug) {
            ...
            EditorGUILayout.TextField("Pan X: " + panX + " PanY: " + panY, GUILayout.MaxWidth(120));
        }
    }
